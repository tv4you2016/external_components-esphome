...
CONF_I2S_ISOUND = "i2s_isound"
CONF_I2S_ISOUND_ID = "i2s_isound_id"
...
i2s_isound_ns = cg.esphome_ns.namespace("i2s_isound")
I2SAudioComponent = i2s_isound_ns.class_("I2SAudioComponent", cg.Component)
I2SAudioBase = i2s_isound_ns.class_(
    "I2SAudioBase", cg.Parented.template(I2SAudioComponent)
)
I2SAudioIn = i2s_isound_ns.class_("I2SAudioIn", I2SAudioBase)
I2SAudioOut = i2s_isound_ns.class_("I2SAudioOut", I2SAudioBase)
...
def i2s_isound_component_schema(
    class_: MockObjClass,
    *,
...
async def register_i2s_isound_component(var, config):
...
def validate_use_legacy(value):
    ...
    if CONF_USE_LEGACY in value:
        if (_use_legacy_driver is not None) and (
            ...
            raise cv.Invalid(
                f"All i2s_isound components must set {CONF_USE_LEGACY} to the same value."
            )
...
def _final_validate(_):
    i2s_isound_configs = fv.full_config.get()[CONF_I2S_ISOUND]
    variant = get_esp32_variant()
    if variant not in I2S_PORTS:
        raise cv.Invalid(f"Unsupported variant {variant}")
    if len(i2s_isound_configs) > I2S_PORTS[variant]:
        raise cv.Invalid(
