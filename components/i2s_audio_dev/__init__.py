...
CONF_I2S_AUDIO_DEV = "i2s_audio_dev"
CONF_I2S_AUDIO_ID_DEV = "i2s_audio_dev_id"
...
i2s_audio_dev_ns = cg.esphome_ns.namespace("i2s_audio_dev")
I2SAudioComponent = i2s_audio_dev_ns.class_("I2SAudioComponent", cg.Component)
I2SAudioBase = i2s_audio_dev_ns.class_(
    "I2SAudioBase", cg.Parented.template(I2SAudioComponent)
)
I2SAudioIn = i2s_audio_dev_ns.class_("I2SAudioIn", I2SAudioBase)
I2SAudioOut = i2s_audio_dev_ns.class_("I2SAudioOut", I2SAudioBase)
...
def i2s_audio_dev_component_schema(
    class_: MockObjClass,
    *,
    ...
    return cv.Schema(
        {
            ...
            cv.GenerateID(CONF_I2S_AUDIO_ID_DEV): cv.use_id(I2SAudioComponent),
...
async def register_i2s_audio_dev_component(var, config):
    await cg.register_parented(var, config[CONF_I2S_AUDIO_ID_DEV])
    if use_legacy():
...
def validate_use_legacy(value):
    ...
    if CONF_USE_LEGACY in value:
        if (_use_legacy_driver is not None) and (
            ...
            raise cv.Invalid(
                f"All i2s_audio_dev components must set {CONF_USE_LEGACY} to the same value."
            )
...
def _final_validate(_):
    i2s_audio_dev_configs = fv.full_config.get()[CONF_I2S_AUDIO_DEV]
    variant = get_esp32_variant()
    if variant not in I2S_PORTS:
        raise cv.Invalid(f"Unsupported variant {variant}")
    if len(i2s_audio_dev_configs) > I2S_PORTS[variant]:
        raise cv.Invalid(
